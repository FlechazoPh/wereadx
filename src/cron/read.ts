...
import {sendTelegramMessage} from "../utils/Message.ts";
/**
 * 执行自动阅读任务
...
export async function runReadTask(_: Request) {
    ...
    for (const task of tasks) {
        ...
        // 先检查 cookie 是不是可能过期
        if (
            Date.now() - credential.updatedAt >= 1000 * (5400 - 30) /* 留30秒空隙 */
        ) {
            // cookie 可能已经过期，尝试刷新
        ...
        console.log(
            ...
            }%c，耗时: ${((Date.now() - taskStartTime) / 1000).toFixed(1)}s`,
    ...
    console.log(
        `全部任务(${taskRunCount}/${tasks.length})执行完毕，耗时: %c${
...
async function getReadingTime(credential: Credential): Promise<number> {
    ...
    if (resp && resp.ranking && Array.isArray(resp.ranking)) {
        ...
        const targetRank = resp.ranking.find((_: any) =>
            _.user.userVid === credential.vid
        );
        if (targetRank) {
            return targetRank.readingTime;
        } else {
            console.warn(
                `没有找到目标用户排名数据(vid:${credential.vid}, name:${credential.name}, skey:${credential.skey})`,
            );
            return -1;
        }
    } else {
...
export async function refreshCookie(credential: Credential) {
    try {
        ...
        const credentialInfo = await web_login_renewal(
            "/web/shelf/sync",
            oldCookie,
        );
